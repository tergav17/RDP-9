...
const STEP_EAE_NORM_GETBIT_B = 4;	// Shift OB[16] into link register
const STEP_EAE_NORM_CHECK_A = 5;	// Check the contents of OB[17]
const STEP_EAE_NORM_CHECK_BX = 6;	// Check contents of OB[16], OB[17] = 0
const STEP_EAE_NORM_CHECK_BY = 7;	// Check contents of OB[16], OB[17] = 1  
const STEP_EAE_NORM_MQ_SHIFT = 8;	// Shift MQ left once
const STEP_EAE_NORM_AC_LOAD = 9;	// Load AC into OB
...
function decode(input) {
	...
	if (decode_mode == DECODE_MODE_SERVICE) {
		...
		switch (step) {
			...
			// OB << 1 -> AC, OB
			// IF FLAG_STEP_ZERO:
			//  STEP_EAE_NORM_LINK_REST -> NEXT
			// ELSE: 
			//  STEP_EAE_NORM_GETBIT_A -> NEXT
			case STEP_SRV_NORM_AC_SHIFT:
				...
				next_decode_mode = DECODE_MODE_EAE;
				if (flag_step_zero) {
					next_step = STEP_EAE_NORM_LINK_REST;
				} else {
			...
			// Otherwise shift AC right once
			// IF FLAG_STEP_ZERO:
			//  STEP_SRV_FETCH -> NEXT
			// ELSE:
			//  OB >> 1 -> AC, OB, FLAG_LINK
			//  STEP_EAE_LRS_MQ_LOAD -> NEXT
			case STEP_SRV_LRS_AC_SHIFT:
				...
				if (flag_step_zero) {
					...
					next_decode_mode = DECODE_MODE_EAE;
					next_step = STEP_EAE_LRS_MQ_LOAD;
				}
				break;
				
			// Check SC, if SC = 0 then do fetch next
			// Otherwise shift MQ left once
			// IF FLAG_STEP_ZERO:
			//  STEP_SRV_FETCH -> NEXT
			// ELSE:
			//  OB << 1 -> MQ, OB, FLAG_LINK
			//  STEP_EAE_LLS_MQ_LOAD -> NEXT
			case STEP_SRV_LLS_MQ_SHIFT:
				...
				switch (step) {
					...
					case STEP_ISR_INDIR_COMPLETE:
						...
						if (indirect && rest_pending) {
							...
							step = STEP_JMP_LOAD_PC;
						}
						
						
					// Perform the jump with the fetched value
					// If it's not indirect, force disabling of extended memory mode
					// IF !INDIR
					//  EXT_OFF = ADDR_REG_MODE
					// (OB OR MB) -> PC
					// STEP_SRV_FETCH -> NEXT
					case STEP_JMP_LOAD_PC:
					
						// Setup ALU mode
						bus_output_select = BUS_SELECT_ALU;
						alu_op_select = ALU_OR;
						
						// Setup the desired addressing mode
						if (indirect) {
							address_register_mode = ADDR_REG_MODE_EXT_OFF;
						}
		...
		if (step < 32) {
			...
			switch (eae_opcode) {
				...
				case EAE_OPCODE_MUL:
					...
					switch (step) {
						...
						// IF FLAG_LINK:
						//  0 -> FLAG_LINK
						//  STEP_EAE_MUL_ADD -> NEXT
						// ELSE:
						//  STEP_EAE_MUL_AC_SHIFT -> NEXT
						case STEP_EAE_MUL_COMP_FLAG:
						...
						// Complement MQ
						// (OB XOR MB) -> MQ
						// STEP_EAE_MUL_AC_LOAD -> NEXT
						case STEP_EAE_MUL_COMP_MQ:
							...
							next_step = STEP_EAE_MUL_AC_LOAD;
							break;
							
						// Load AC into MB
						// AC -> MB
						// STEP_EAE_MUL_COMP_AC -> NEXT
						case STEP_EAE_MUL_AC_LOAD:
						
							// AC -> MB
							bus_output_select = BUS_SELECT_AC;
							latch_mb = 1;
						
							next_step = STEP_EAE_MUL_COMP_AC;
							break;
				...
				case EAE_OPCODE_NORM:
					...
					switch (step) {
						...
						// Negate SC
						// (OB XOR MB) + 1 -> SC
						// STEP_EAE_NORM_AC_INIT -> NEXT
						case STEP_EAE_NORM_NEG_SC:
							...
							next_step = STEP_EAE_NORM_AC_INIT;
							break;

						// Load AC into OB
						// AC -> OB
						// STEP_EAE_NORM_GETBIT_A -> NEXT
						case STEP_EAE_NORM_AC_INIT:
						...
						case STEP_EAE_NORM_AC_LOAD:
							...
							next_decode_mode = DECODE_MODE_SERVICE;
							next_step = STEP_SRV_NORM_AC_SHIFT;
							break;
							
						// Restore link flag prior to SC exit
						// AC -> OB
						// FLAG_LINK_INIT -> FLAG_LINK
						case STEP_EAE_NORM_LINK_REST:
						...
						default:
							// Invalid step, stop instruction execution
							console.log("Warning: We tried to decode an unimplemented EAE normalize step!");
							next_decode_mode = DECODE_MODE_SERVICE;
							next_step = STEP_SRV_FETCH;
							break;
				...
				case EAE_OPCODE_LRS:
					...
					switch (step) {
						...
						// Negate SC
						// (OB XOR MB) + 1 -> SC
						// STEP_EAE_LRS_LINK_REST -> NEXT
						case STEP_EAE_LRS_NEG_SC:
							...
							next_step = STEP_EAE_LRS_LINK_REST;
							break;
						...
						case STEP_EAE_LRS_LINK_REST:
							...
							next_decode_mode = DECODE_MODE_SERVICE;
							next_step = STEP_SRV_LRS_AC_SHIFT;
							break;
						...
						// MQ -> OB
						// SC + 1 -> SC
						// STEP_EAE_LRS_MQ_SHIFT -> NEXT
						case STEP_EAE_LRS_MQ_LOAD:
							...
							next_step = STEP_EAE_LRS_MQ_SHIFT;
							break;
							
						// Shift MQ right once
						// OB >> 1 -> OB, MQ
						// STEP_EAE_LRS_LINK_REST -> NEXT
						case STEP_EAE_LRS_MQ_SHIFT:
						...
						default:
							// Invalid step, stop instruction execution
							console.log("Warning: We tried to decode an unimplemented EAE LRS step!");
							next_decode_mode = DECODE_MODE_SERVICE;
							next_step = STEP_SRV_FETCH;
							break;
					}
				...
				case EAE_OPCODE_LLS:
					...
					switch (step) {
						...
						case STEP_EAE_LLS_NEG_SC:
							...
							next_step = STEP_EAE_LLS_LINK_REST;
							break;
						...
						case STEP_EAE_LLS_LINK_REST:
							...
							next_decode_mode = DECODE_MODE_SERVICE;
							next_step = STEP_SRV_LLS_MQ_SHIFT;
							break;
						...
						// AC -> OB
						// SC + 1 -> SC
						// STEP_EAE_LLS_AC_SHIFT -> NEXT
						case STEP_EAE_LLS_AC_LOAD:
						...
						default:
							// Invalid step, stop instruction execution
							console.log("Warning: We tried to decode an unimplemented EAE LLS step!");
							next_decode_mode = DECODE_MODE_SERVICE;
							next_step = STEP_SRV_FETCH;
							break;
					}
			...
			switch (step) {
				...
				// 0777777 -> OB
				// IF FLAG_EAE_AC_SIGN:
				//  STEP_EAE_MUL_NACS -> NEXT
				// ELSE:
				//  STEP_EAE_MUL_PACS -> NEXT
